...
class RelationshipEngineTests(unittest.TestCase):
    ...
    def test_discovers_candidate_relationships(self):
        ...
        self.assertIn((ORDERS_TABLE, "product_id", PRODUCTS_TABLE, "product_id"), pairs)
        self.assertIn((ORDERS_TABLE, "sales_id", EMPLOYEES_TABLE, "sales_id"), pairs)
    ...
    def test_recommends_dates_but_excludes_amount_from_connectable_fields(self):
        ...
        orders_table = next(
            ...
            if table["table_name"] == ORDERS_TABLE
        )
    ...
    def test_saves_loads_and_syncs_relationships_to_project(self):
        ...
        relationship = {
            "table_a_id": tables[ORDERS_TABLE]["table_id"],
            "table_a_name": tables[ORDERS_TABLE]["table_name"],
            "table_a_file_id": tables[ORDERS_TABLE]["file_id"],
            "table_a_file_name": tables[ORDERS_TABLE]["file_name"],
            "table_a_sheet_name": tables[ORDERS_TABLE]["sheet_name"],
            "field_a": "product_id",
            "table_b_id": tables[PRODUCTS_TABLE]["table_id"],
            "table_b_name": tables[PRODUCTS_TABLE]["table_name"],
            "table_b_file_id": tables[PRODUCTS_TABLE]["file_id"],
            "table_b_file_name": tables[PRODUCTS_TABLE]["file_name"],
            "table_b_sheet_name": tables[PRODUCTS_TABLE]["sheet_name"],
            "field_b": "product_id",
    ...
    def test_loads_legacy_main_target_relationship_as_table_a_table_b(self):
        tables = {table["table_name"]: table for table in list_project_tables(self.project["project_id"])}
        legacy = {
            "main_table_id": tables[ORDERS_TABLE]["table_id"],
            "main_table_name": "orders",
            "main_column": "sales_id",
            "target_table_id": tables[EMPLOYEES_TABLE]["table_id"],
    ...
    def test_deletes_single_relationship_and_syncs_project(self):
        ...
        second = {
            ...
            "table_a_name": tables[ORDERS_TABLE]["table_name"],
            "field_a": "sales_id",
            "table_b_id": tables[EMPLOYEES_TABLE]["table_id"],
            "table_b_name": tables[EMPLOYEES_TABLE]["table_name"],
            "field_b": "sales_id",
            "relationship_type": "many_to_one",
        }
